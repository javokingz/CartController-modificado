...
class CartControllerCore extends FrontController
{
    ...
    protected function processChangeProductInCart()
    {
        ...
        //agragamos una variable igualada a cero
        $total_cart_products_count = 0;

        if (is_array($cart_products)) {
            foreach ($cart_products as $cart_product) {

                //A la variable creada le sumamos la cantidad del carrito
        ...
        //Añadimos una variable con la cantidad maxima en este caso 4
        $max_quantity = 4;
        if(Tools::getValue('op', 'up') == 'up' && $mode == 'add')
        {
            //Contamos los productos añadidos
            if(count($cart_product) == $max_quantity || $total_cart_products_count + $this->qty > $max_quantity)
               
                $this->error[] = sprintf(Tools::displayError('No puedes agregar mas de %s al carrito'), $max_quantity);
        }
